...
export async function POST(request: Request) {
  ...
  try { getMission(body.missionId); } catch { return NextResponse.json({ error: "unknown mission" }, { status: 400 }); }
  ...
  try {
    ...
    return NextResponse.json({ mode: "real", executed: true, result: await executeLab(identity.id, body.missionId, body.command) });
  } catch (error) {

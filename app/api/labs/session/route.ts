...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ mode: "real", ready: true, missionId: mission.id, session: await ensureLab(identity.id, mission.id), liveValidator: mission.liveValidator ?? null });
  } catch (error) {

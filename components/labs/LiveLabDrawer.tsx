...
export function LiveLabDrawer({ missionId, validationEnabled = false, onValidated }: { missionId: string; validationEnabled?: boolean; onValidated?: (payload: ValidationPayload) => void }) {
  const [open, setOpen] = useState(false);
  const [capability, setCapability] = useState<"loading"|"vercel"|"simulated">("loading");
  const [status, setStatus] = useState<"idle"|"starting"|"ready"|"running"|"validating"|"validated"|"stopped"|"error">("idle");
  ...
  return <div className={`live-lab ${open?"open":""}`}>
    ...
    {open&&<div className="live-lab-body">
      ...
      <div className="live-lab-actions"><button onClick={start} disabled={status==="starting"}>{status==="starting"?"Starting…":status==="ready"||status==="validated"?"Resume":"Start session"}</button>{validationEnabled&&<button onClick={validate} disabled={capability!=="vercel"||status==="validating"||status==="validated"}>{status==="validating"?"Validating…":status==="validated"?"Validated ✓":"Validate live environment"}</button>}<button onClick={stop}>Stop</button></div>
